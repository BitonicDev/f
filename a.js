...
openXsmMenu: function() {
    ...
    // --- Prefix presets definition ---
    // Each entry: { label, names[], prefixed (bool) }
    const PREFIX_PRESETS = [
        { value: "abc",  label: "abc",  names: ["a","b","c"],         prefixed: true  },
        { value: "xsm",  label: "xsm",  names: ["x","s","m"],         prefixed: true  },
    ...
    let presetOptions = `<option value="">— none (use name list) —</option>`;
    for (let p of PREFIX_PRESETS) {
        presetOptions += `<option value="${p.value}">${p.label} → ${p.names.join(", ")}</option>`;
    }
    ...
    menu.innerHTML = `
        ...
        <div style="padding:10px 12px;">
            ...
            <div style="margin-bottom:8px;">
                <label style="font-size:11px;color:#aaa;margin-bottom:4px;display:block;">Prefix preset:</label>
                <select id="xsm-preset" style="width:100%;padding:6px 7px;background:#111;color:#fff;border:1px solid #333;border-radius:4px;font-size:12px;outline:none;">
                    ${presetOptions}
                </select>
            </div>

            <div style="display:flex;align-items:center;gap:7px;margin-bottom:8px;">
                <input type="checkbox" id="xsm-prefix-check" checked style="margin:0;width:13px;height:13px;flex-shrink:0;accent-color:#2ecc71;">
                <label for="xsm-prefix-check" style="font-size:12px;color:#ccc;cursor:pointer;">Add <code style="background:#333;padding:1px 4px;border-radius:3px;font-size:11px;">n:</code> prefix to all names</label>
            </div>

            <div id="xsm-names-container" style="max-height:130px;overflow-y:auto;margin-bottom:8px;padding-right:3px;"></div>

            <button id="xsm-add-btn" style="width:100%;padding:6px;background:#3a3a4a;color:#fff;border:1px solid #4a4a5a;border-radius:5px;cursor:pointer;margin-bottom:10px;font-weight:bold;font-size:12px;">+ Add Name</button>

            <div style="margin-bottom:10px;">
                <label style="font-size:11px;color:#aaa;margin-bottom:4px;display:block;">Target Clan ID:</label>
                <input type="number" id="xsm-clan-id" placeholder="e.g. 0" style="width:100%;padding:6px 7px;box-sizing:border-box;background:#111;color:#fff;border:1px solid #333;border-radius:5px;outline:none;font-size:12px;">
            </div>
            ...
            <div id="xsm-status" style="margin-top:8px;font-size:11px;text-align:center;color:#f1c40f;font-weight:bold;min-height:14px;"></div>
        </div>
    `;
    ...
    // --- Helpers ---
    const container = menu.querySelector("#xsm-names-container");
    const prefixCheck = menu.querySelector("#xsm-prefix-check");
    const presetSelect = menu.querySelector("#xsm-preset");
    ...
    const makeRow = (value = "") => {
        ...
        row.innerHTML = `
            <input type="text" placeholder="Bot Name" class="xsm-name-input" value="${value}" style="flex:1;padding:5px 7px;background:#111;color:#fff;border:1px solid #333;border-radius:4px;outline:none;font-size:12px;">
    ...
    const setRows = (names) => {
        container.innerHTML = "";
        for (let n of names) container.appendChild(makeRow(n));
    };

    // Start with one blank row
    setRows([""]);

    // Preset change: replace name list and sync checkbox
    presetSelect.onchange = () => {
        let preset = PREFIX_PRESETS.find(p => p.value === presetSelect.value);
        if (preset) {
            setRows(preset.names);
            prefixCheck.checked = preset.prefixed;
        } else {
            setRows([""]);
        }
    };
    ...
    menu.querySelector("#xsm-add-btn").onclick = () => {
        // Deselect preset if user is manually editing
        presetSelect.value = "";
    ...
    const onMouseMove = (e) => { if (!isDragging) return; menu.style.left = (e.clientX - offsetX) + "px"; menu.style.top = (e.clientY - offsetY) + "px"; };
    const onMouseUp = () => { isDragging = false; header.style.cursor = "grab"; };
    header.addEventListener("mousedown", (e) => {
        isDragging = true;
        let rect = menu.getBoundingClientRect();
        if (menu.style.transform) { menu.style.transform = "none"; menu.style.left = rect.left + "px"; menu.style.top = rect.top + "px"; }
    ...
    menu.querySelector("#xsm-run-btn").onclick = async () => {
        ...
        if (isNaN(clanId) || clanId < 0) { status.innerText = "Invalid Clan ID!"; return; }

        let usePrefix = prefixCheck.checked;

        // Collect names in DOM order (top → bottom)
        let rawNames = Array.from(container.querySelectorAll(".xsm-name-input"))
            .map(i => i.value.trim())
            .filter(n => n.length > 0);

        if (rawNames.length === 0) { status.innerText = "No names provided!"; return; }
