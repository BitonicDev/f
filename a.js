...
// === AUTOPLAY BOT ===
    const autoplayBot = {
        active: false, targetX: 0, targetY: 0, path: [], lastPathTime: 0, gridSize: 100,
        mapSize: _0xca1cdc.KN || 10000, gridOffset: 1000, streamTargetId: null, lastStreamTime: 0,
        streamInterval: 5000, autoEmpActive: false, isEmpForced: false, prevHatId: -1,
        autoCactusActive: false, autoChatActive: false, autoEatActive: false, zoomVal: 1, lastChatTime: 0,
        reqmatsRemaining: 0, lastReqmatsTime: 0, autoRespawnActive: false, noIframesActive: false,
        coordsActive: true, detailsActive: false, iframeDropped: false, iframeDropping: false,
        coordsEl: null, coordsText: null, detailsEl: null, detailsText: null, frameCount: 0,
        currentFps: 0, lastFpsTime: Date.now(), lastPingTime: 0, currentPing: 0, waitingForPing: false,

        getBatId: () => _0xca1cdc.ev.find(e => e && e.name === "Bat")?.id || 2,
        worldToGrid: function(val) { return Math.floor((val + this.gridOffset) / this.gridSize); },
        gridToWorld: function(val) { return (val * this.gridSize) - this.gridOffset + (this.gridSize / 2); },

        openXsmMenu: function() {
            if (document.getElementById("xsm")) return;
            let m = document.createElement("div");
            m.id = "xsm";
            m.style.cssText = "position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);background:#222;padding:15px;color:#fff;z-index:9999;border:1px solid #555;display:flex;flex-direction:column;gap:5px;width:250px;";
            m.innerHTML = `
                <h3 style="margin:0;text-align:center;">XSM Clan Bot</h3>
                <div id="xsm-n" style="display:flex;flex-direction:column;gap:5px;max-height:200px;overflow:auto;">
                    <div style="display:flex;"><input class="xn" placeholder="Name" style="flex:1;"><button onclick="this.parentElement.remove()">X</button></div>
                </div>
                <button onclick="let d=document.createElement('div');d.style.display='flex';d.innerHTML='<input class=\\'xn\\' placeholder=\\'Name\\' style=\\'flex:1\\'><button onclick=\\'this.parentElement.remove()\\'>X</button>';document.getElementById('xsm-n').appendChild(d);">+ Add Name</button>
                <input id="xsm-c" type="number" placeholder="Clan ID">
                <div style="display:flex;gap:5px;margin-top:5px;">
                    <button id="xsm-r" style="flex:1;background:#27ae60;color:#fff;border:none;padding:5px;">Run</button>
                    <button onclick="document.getElementById('xsm').remove()" style="flex:1;">Close</button>
                </div>`;
            document.body.appendChild(m);

            m.querySelector("#xsm-r").onclick = async () => {
                let url = _0x419dc9?.url;
                if (!url) return alert("Not connected to a server!");
                let names = Array.from(m.querySelectorAll(".xn")).map(i => i.value.trim()).filter(v => v);
                let cid = parseInt(m.querySelector("#xsm-c").value);
                if (!names.length || isNaN(cid)) return alert("Invalid names or Clan ID!");
                
                m.querySelector("#xsm-r").innerText = "Connecting...";
                let bots = [];

                // 1. Connect and wait for all to fully authenticate
                await Promise.all(names.map((name, i) => new Promise(res => {
                    let ws = new WebSocket(url);
                    ws.binaryType = "arraybuffer";
                    let uuid = crypto.randomUUID(), xorKey = 0;
                    
                    ws.onopen = () => {
                        let buf = new ArrayBuffer(39), view = new DataView(buf);
                        view.setUint8(0, 5); view.setUint16(1, 0x151); // 5 = login
                        for(let j=0; j<36; j++) view.setUint8(3+j, uuid.charCodeAt(35-j) ^ (0x151 % 255));
                        xorKey = view.getUint8(3 + (0x151 % 36));
                        ws.send(buf);
                    };
                    ws.onmessage = (msg) => {
                        if (new Uint8Array(msg.data)[0] === 3) { // 3 = loggedIn
                            bots[i] = { ws, xorKey, name };
                            res();
                        }
                    };
                    ws.onerror = ws.onclose = res;
                    setTimeout(res, 5000); // 5s timeout fallback
                })));

                // 2. Instantly execute all of them in sequential array order, then drop connection
                bots.forEach(bot => {
                    if (!bot || bot.ws.readyState !== 1) return;
                    let send = (pkt) => {
                        let s = new Uint8Array(pkt);
                        for(let j=0; j<s.length; j++) s[j] ^= bot.xorKey;
                        let swp = 0x151 % s.length, t = s[0];
                        s[0] = s[swp]; s[swp] = t;
                        bot.ws.send(s);
                    };
                    
                    let nb = new TextEncoder().encode(bot.name.slice(0, 16));
                    let sp = new Uint8Array(2 + nb.length); // spawn packet
                    sp[0] = 0; sp[1] = Math.floor(Math.random()*6);
                    sp.set(nb, 2);
                    
                    send(sp);          // Join Game (Spawn)
                    send([6, cid]);    // Join Clan Request
                    bot.ws.close();    // Disconnect Instantly
                });

                m.querySelector("#xsm-r").innerText = "Done!";
                setTimeout(() => m.querySelector("#xsm-r").innerText = "Run", 2000);
            };
        },

        handleCommand: function(msg) {
            if (!msg) return;
            let parts = msg.trim().split(" "), cmd = parts[0].toLowerCase();

            if (cmd === "/help") {
                alert("Commands: /xsm, /goto [x] [y], /stop, /leave [id], /stream [id], /gift [id] [pct], /account [user], /autoemp [bool], /autocactus [bool], /autochat [bool], /autoeat [bool], /autorespawn [bool], /noiframes [bool], /coords [bool], /details [bool], /zoom [val], /reqmats [count], /kick [id], /deleteclan, /server [alias/url]");
            }
            else if (cmd === "/xsm") this.openXsmMenu();
            else if (cmd === "/goto" && parts.length >= 3) { this.targetX = parseFloat(parts[1]); this.targetY = parseFloat(parts[2]); this.active = true; } 
            else if (cmd === "/stop") { this.active = false; this.streamTargetId = null; this.autoEmpActive = false; this.autoChatActive = false; this.autoEatActive = false; this.reqmatsRemaining = 0; this.resetKeys(); } 
            else if (cmd === "/leave" && parts[1]) { if (this.sendGift(parseInt(parts[1]), 1.0)) setTimeout(() => window.connect && window.connect(_0x419dc9.url), 100); }
            else if (cmd === "/stream" && parts[1]) this.streamTargetId = parseInt(parts[1]);
            else if (cmd === "/gift" && parts[1]) this.sendGift(parseInt(parts[1]), (parseFloat(parts[2])||50)/100);
            else if (cmd === "/account" && parts[1]) __originalSend(new Uint8Array([_0xca1cdc.wT.iAccountDataReq, ...new TextEncoder().encode(parts.slice(1).join(" "))]));
            else if (cmd === "/autoemp" && parts[1]) { this.autoEmpActive = parts[1]==="true"; this.isEmpForced = false; if(this.autoEmpActive && _0x466240) this.prevHatId = _0x466240.skin?.id || -1; }
            else if (cmd === "/autocactus" && parts[1]) this.autoCactusActive = parts[1]==="true";
            else if (cmd === "/autochat" && parts[1]) { this.autoChatActive = parts[1]==="true"; this.lastChatTime = 0; }
            else if (cmd === "/autoeat" && parts[1]) this.autoEatActive = parts[1]==="true";
            else if (cmd === "/autorespawn" && parts[1]) this.autoRespawnActive = parts[1]==="true";
            else if (cmd === "/noiframes" && parts[1]) this.noIframesActive = parts[1]==="true";
            else if (cmd === "/coords" && parts[1]) { this.coordsActive = parts[1]==="true"; if (this.coordsEl) this.coordsEl.style.display = this.coordsActive ? "" : "none"; }
            else if (cmd === "/details" && parts[1]) { this.detailsActive = parts[1]==="true"; if (this.detailsEl) this.detailsEl.style.display = this.detailsActive ? "" : "none"; }
            else if (cmd === "/zoom" && parts[1]) this.zoomVal = parseFloat(parts[1]) || this.zoomVal;
            else if (cmd === "/reqmats") { this.reqmatsRemaining = parseInt(parts[1])||1; this.lastReqmatsTime = 0; }
            else if (cmd === "/kick" && parts[1]) __originalSend(new Uint8Array([_0xca1cdc.wT.iClanKick, parseInt(parts[1])]));
            else if (cmd === "/deleteclan") { let m = _0x311e82.children; if (m) for (let i=m.length-1; i>=0; i--) if (!m[i].isMe) __originalSend(new Uint8Array([_0xca1cdc.wT.iClanKick, i])); __originalSend(new Uint8Array([_0xca1cdc.wT.iClanLeave])); }
            else if (cmd === "/server" && parts[1]) { let u={"eu1":"wss://eu.poopoo.pro","eu2":"wss://eu2.poopoo.pro","us1":"wss://us.poopoo.pro","us2":"wss://us2.poopoo.pro"}[parts[1].toLowerCase()] || parts[1]; if(window.connect) window.connect(u); }
        },

        sendGift: function(targetId, mult = 1.0) {
            let members = _0x311e82.children; 
            if (members.length <= 1 || targetId < 0 || targetId >= members.length || members[targetId].isMe) return false;
            let f = Math.floor(_0x5cb651 * mult), w = Math.floor(_0x475a45 * mult), s = Math.floor(_0x505bb2 * mult), g = Math.floor(_0x4e3cab * mult);
            let mask = 0, vals = [];
            if (f > 0) { mask |= 1; vals.push(f); } if (w > 0) { mask |= 2; vals.push(w); } 
            if (s > 0) { mask |= 4; vals.push(s); } if (g > 0) { mask |= 8; vals.push(g); } 
            if (mask > 0) {
                let view = new DataView(new ArrayBuffer(3 + vals.length * 4));
                view.setUint8(0, _0xca1cdc.wT.iGift); view.setUint8(1, targetId); view.setUint8(2, mask);
                vals.forEach((v, i) => view.setUint32(3 + i * 4, v));
                __originalSend(new Uint8Array(view.buffer));
                return true;
            }
            return false;
        },
        ...
        resetKeys: function() {
            _0x4cfb62.KeyW = _0x4cfb62.KeyS = _0x4cfb62.KeyA = _0x4cfb62.KeyD = _0x4cfb62.mouse0 = _0x4cfb62.ShiftLeft = false;
            _0x5629b9(); 
        },
        ...
        buildGrid: function() {
            ...
            for (let i = 0; i < _0x5a712e.length; i++) {
                let ent = _0x5a712e[i];
                if (ent === _0x466240 || ent.isDead || ent.isPlayer || ent.isProj || ent.isDisplay || ent.isHole || ent.type === 6) continue;
                let cx = this.worldToGrid(ent.x), cy = this.worldToGrid(ent.y), rad = Math.ceil(((ent.size || 40) + 40) / this.gridSize); 
                for (let dx = -rad; dx <= rad; dx++) for (let dy = -rad; dy <= rad; dy++) {
                    let nx = cx + dx, ny = cy + dy;
                    if (nx >= 0 && nx < cells && ny >= 0 && ny < cells) grid[nx + ny * cells] = 1; 
                }
            }
            return { grid, cells };
        },
        ...
        aStar: function(startX, startY, goalX, goalY) {
            ...
            startX = Math.max(0, Math.min(cells - 1, startX)); startY = Math.max(0, Math.min(cells - 1, startY));
            goalX = Math.max(0, Math.min(cells - 1, goalX)); goalY = Math.max(0, Math.min(cells - 1, goalY));
            let openSet = [{x: startX, y: startY, g: 0, f: 0, parent: null}], closedSet = new Uint8Array(cells * cells);
            while(openSet.length > 0) {
                let minIndex = 0;
                for (let i = 1; i < openSet.length; i++) if (openSet[i].f < openSet[minIndex].f) minIndex = i;
                let current = openSet.splice(minIndex, 1)[0];
                if (current.x === goalX && current.y === goalY) {
                    let path = [], curr = current;
                ...
                for(let n of neighbors) {
                    let nx = current.x + n.x, ny = current.y + n.y;
                    if(nx < 0 || ny < 0 || nx >= cells || ny >= cells || grid[nx + ny * cells] === 1) continue; 
                    openSet.push({x: nx, y: ny, g: current.g + (n.x !== 0 && n.y !== 0 ? 1.414 : 1), f: current.g + (n.x !== 0 && n.y !== 0 ? 1.414 : 1) + (Math.hypot(goalX - nx, goalY - ny) * 1.2), parent: current});
                }
            }
            return null;
        },
        ...
        update: function() {
            let now = Date.now();
            if (this.waitingForPing && window.lastPacketTime > this.lastPingTime) { this.currentPing = window.lastPacketTime - this.lastPingTime; this.waitingForPing = false; }
            if (_0x466240) {
                if (!_0x466240.skin) _0x466240.skin = { name: "mock_zoom", id: -1, viewScale: this.zoomVal };
                else {
                    if (typeof _0x466240.skin.origViewScale === "undefined") _0x466240.skin.origViewScale = _0x466240.skin.viewScale || 1;
                    _0x466240.skin.viewScale = _0x466240.skin.origViewScale * this.zoomVal;
                }
            }

            if (this.autoRespawnActive && !_0x466240 && (!(_0x2885af && _0x2885af.style.display !== "none"))) {
                if (_0x17eee1 && _0x17eee1.style.display === "") { if (_0x2867cc) _0x2867cc.click(); } 
                else if (_0x2fc9ad && _0x2fc9ad.style.display === "") { if (_0x3ee957) _0x3ee957.click(); }
            }

            if (this.noIframesActive && _0x466240 && _0x466240.spawnImmunity && !this.iframeDropped && !this.iframeDropping) {
                this.iframeDropping = true;
                let currentState = (_0x4cfb62.KeyW || _0x4cfb62.ArrowUp ? 1 : 0) | (_0x4cfb62.KeyS || _0x4cfb62.ArrowDown ? 2 : 0) | (_0x4cfb62.KeyA || _0x4cfb62.ArrowLeft ? 4 : 0) | (_0x4cfb62.KeyD || _0x4cfb62.ArrowRight ? 8 : 0) | (_0x12d5b2 || _0x4cfb62.Space || _0x4cfb62.mouse0 ? 16 : 0) | (_0x4cfb62.ShiftLeft || _0x4cfb62.ShiftRight ? 32 : 0) | (_0x4cfb62.mouse2 || _0x5d3d11 ? 64 : 0);
                __originalSend(new Uint8Array([1, currentState | 0x10])); 
                setTimeout(() => {
                    if (_0x466240) __originalSend(new Uint8Array([1, (_0x4cfb62.KeyW || _0x4cfb62.ArrowUp ? 1 : 0) | (_0x4cfb62.KeyS || _0x4cfb62.ArrowDown ? 2 : 0) | (_0x4cfb62.KeyA || _0x4cfb62.ArrowLeft ? 4 : 0) | (_0x4cfb62.KeyD || _0x4cfb62.ArrowRight ? 8 : 0) | (_0x12d5b2 || _0x4cfb62.Space || _0x4cfb62.mouse0 ? 16 : 0) | (_0x4cfb62.ShiftLeft || _0x4cfb62.ShiftRight ? 32 : 0) | (_0x4cfb62.mouse2 || _0x5d3d11 ? 64 : 0)])); 
                    this.iframeDropped = true; this.iframeDropping = false;
                }, 5);
            } else if (_0x466240 && !_0x466240.spawnImmunity) { this.iframeDropped = this.iframeDropping = false; }

            let mkUI = (cls, propObj, propTxt) => {
                let kcSpan = document.querySelector(".kill-count span");
                if (kcSpan && kcSpan.parentNode && kcSpan.parentNode.parentNode && !this[propObj]) {
                    this[propObj] = document.createElement("div");
                    this[propObj].className = `kill-count ${cls}`;
                    this[propObj].style.marginBottom = "5px";
                    this[propTxt] = document.createElement("span");
                    this[propObj].appendChild(this[propTxt]);
                    kcSpan.parentNode.parentNode.insertBefore(this[propObj], kcSpan.parentNode);
                }
            };

            if (this.coordsActive && _0x466240) {
                mkUI("bot-coords", "coordsEl", "coordsText");
                if (this.coordsEl) { this.coordsEl.style.display = ""; _0x5a0152(this.coordsText, `📌 ${Math.round(_0x466240.x)}, ${Math.round(_0x466240.y)}`); }
            } else if (this.coordsEl) this.coordsEl.style.display = "none";

            if (this.detailsActive && _0x466240) {
                mkUI("bot-details", "detailsEl", "detailsText");
                if (this.detailsEl) { this.detailsEl.style.display = ""; _0x5a0152(this.detailsText, `📊 FPS: ${this.currentFps} | Ping: ${this.currentPing}ms | Ent: ${_0x5a712e ? _0x5a712e.length : 0}`); }
            } else if (this.detailsEl) this.detailsEl.style.display = "none";

            if (this.reqmatsRemaining > 0 && now - this.lastReqmatsTime >= 30500) {
                __originalSend(new Uint8Array([_0xca1cdc.wT.iReqMats])); 
                this.reqmatsRemaining--; this.lastReqmatsTime = now;
            }

            if (this.autoChatActive && now - this.lastChatTime >= 1000) {
                __originalSend(new Uint8Array([_0xca1cdc.wT.iChat, ...new TextEncoder().encode(Math.floor(Math.random() * 16777215).toString(16).padStart(6, '0'))]));
                this.lastChatTime = now;
            }

            if (this.autoEatActive) {
                _0x4cfb62.ShiftLeft = _0x4cfb62.mouse0 = true;
                _0x5629b9(); 
                __originalSend(new Uint8Array([_0xca1cdc.wT.iChangeItem, 4])); 
            }

            if (this.autoEmpActive && _0x466240) {
                let snipers = 0, cannons = 0;
                for (let i = 0; i < _0x5a712e.length; i++) {
                    let ent = _0x5a712e[i];
                    if (ent.isCannon && !ent.isDead && Math.hypot(_0x466240.x - ent.x, _0x466240.y - ent.y) < 600) {
                        ent.type === 24 ? snipers++ : cannons++;
                    }
                }
                let shouldEmp = snipers > 0 || cannons > 4;
                if (shouldEmp && !this.isEmpForced) {
                    this.prevHatId = _0x466240.skin?.name !== "EMP" ? (_0x466240.skin?.id ?? -1) : -1;
                    __originalSend(new Uint8Array([_0xca1cdc.wT.iChangeSkin, 9]));
                    this.isEmpForced = true;
                } else if (!shouldEmp && this.isEmpForced) {
                    __originalSend(new Uint8Array([_0xca1cdc.wT.iChangeSkin, this.prevHatId !== -1 ? this.prevHatId + 1 : 0]));
            ...
            if (this.streamTargetId !== null && now - this.lastStreamTime > this.streamInterval) {
                let m = _0x311e82.children;
                if (this.streamTargetId >= m.length || m.length <= 1) this.streamTargetId = null;
                else { this.sendGift(this.streamTargetId, 1.0); this.lastStreamTime = now; }
            }
            ...
            if (this.autoCactusActive && _0x5c84b0 && _0x5c84b0.classList.contains("show")) {
                ...
                if (f > 0 || w > 0 || s > 0 || g > 0) {
                    let view = new DataView(new ArrayBuffer(17));
                    view.setUint8(0, _0xca1cdc.wT.iWithdraw); view.setUint32(1, f); view.setUint32(5, w); view.setUint32(9, s); view.setUint32(13, g);
                    __originalSend(new Uint8Array(view.buffer));
                    _0x53bf5e[0] = _0x53bf5e[1] = _0x53bf5e[2] = _0x53bf5e[3] = 0;
            ...
            if (Math.hypot(this.targetX - myX, this.targetY - myY) <= 50) { this.active = false; this.resetKeys(); return; }
            ...
            if (this.path && this.path.length > 0) {
                let wp = this.path[0]; wpX = this.gridToWorld(wp.x); wpY = this.gridToWorld(wp.y);
                if (Math.hypot(wpX - myX, wpY - myY) < this.gridSize && this.path.length > 1) {
                    this.path.shift(); wp = this.path[0]; wpX = this.gridToWorld(wp.x); wpY = this.gridToWorld(wp.y);
                }
                if (this.path.length === 1) { wpX = this.targetX; wpY = this.targetY; }
            }
            let dx = wpX - myX, dy = wpY - myY, bd = 60, bId = this.getBatId();
            let isDig = ((myX < bd && dx < 0) || (myX > this.mapSize - bd && dx > 0)) || ((myX < 0 && myX > -bd && dx > 0) || (myX > this.mapSize && myX < this.mapSize + bd && dx < 0)) || ((myY < bd && dy < 0) || (myY > this.mapSize - bd && dy > 0)) || ((myY < 0 && myY > -bd && dy > 0) || (myY > this.mapSize && myY < this.mapSize + bd && dy < 0));
            if (isDig) { if (_0x466240.item?.id !== bId) _0xb6b4d7(bId); _0x4cfb62.mouse0 = true; _0x965f53(Math.atan2(dy, dx)); } else _0x4cfb62.mouse0 = false;
    ...
    const countFps = () => {
        ...
        if (Date.now() - autoplayBot.lastFpsTime >= 1000) { autoplayBot.currentFps = autoplayBot.frameCount; autoplayBot.frameCount = 0; autoplayBot.lastFpsTime = Date.now(); }
    ...
    document.addEventListener("keydown", (e) => {
        if (e.altKey && e.code === "KeyK") {
            e.preventDefault();
            let cmd = prompt("Enter bot command (e.g. /xsm, /zoom 1.5, /autorespawn true, /reqmats 5, /server eu2):");
    ...
    _0x2d5e24 = function(packet) {
        if (packet[0] === _0xca1cdc.wT.iChat && new TextDecoder().decode(packet.slice(1)).startsWith("/")) return autoplayBot.handleCommand(new TextDecoder().decode(packet.slice(1)));
        if (packet[0] === _0xca1cdc.wT.iPing) { autoplayBot.lastPingTime = Date.now(); autoplayBot.waitingForPing = true; }
